		...
		try {
			...
			switch(command) {
				...
				case 'meme':
					...
					break
			    case 'oi':
		            if (args.length < 1) return reply ('Oie gostosa')
		            break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSIKgetZSQWGN8aIbEPB_F5yFCdqXdyNP8dzw&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Nome:* Sadzinho\n*Canal YT:* https://youtube.com/c/SADZMODZ\n*insta:* sadzinhomaker'})
					break
				/*case 'memeindo':
					memein = await kagApi.memeindo()
